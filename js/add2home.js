...
google.bookmarkbubble.Bubble.prototype.build_ = function() {
  ...
  if (isIpad) {
    ...
    arrow.style.top = '-19px';
		...
		if(screen.width==320)
		{
 			alert(screen.height+ 'Small Device ' +screen.width);
             		if(screen.height<screen.width)
       			 {
   			 alert('Android Small Device LandScape mode');
    			  bubble.style.left = '0px';
     			 bubble.style.top = '-505px';
    			 arrow.style.WebkitTransform = 'rotate(180deg)';
    			 arrow.style.top = '-19px';
    			}else
   			 {
    			 alert('Android Small Device port mode');
     			 bubble.style.left = '28px';
     			 bubble.style.top = '-260px';
    			 arrow.style.WebkitTransform = 'rotate(180deg)';
     			arrow.style.top = '-19px';
			arrow.style.left = '203px';
 			
			}
		}else
{

    alert(screen.height+ 'Large device ' +screen.width);
                  if(screen.height<screen.width)
        {
    alert('Land');
      bubble.style.left = '495px';
      bubble.style.top = '-505px';
     arrow.style.WebkitTransform = 'rotate(180deg)';
     arrow.style.top = '-19px';
    }else
    {
     alert('port');
      bubble.style.left = '255px';
      bubble.style.top = '-985px';
     arrow.style.WebkitTransform = 'rotate(180deg)';
     arrow.style.top = '-19px';
}
}
